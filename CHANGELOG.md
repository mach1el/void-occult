...
- **Tử Vi**: Module experimental Khí vận 12 cung (palace-overview V1) — phân tích cấu trúc nguyên cục tĩnh; mặc định tắt, bật bằng `VITE_ZIWEI_PALACE_OVERVIEW_V1` hoặc `?ziweiPalaceOverviewV1=1`.
- **Tử Vi**: Khung module phân tích vận khí mới (`palace-overview` · `annual-axes` · `major-fortune` · `monthly-flow`) — các module chấm điểm experimental lần lượt lên; `monthly-flow` V0 đã có engine API (chưa UI).
...
### Sửa Lỗi (Fixed)
- **Tử Vi**: Siết contract Monthly Flow V0 — năm thiếu/trùng tháng không còn báo `available`; Tứ Hóa tháng chỉ khớp sao gốc vật lý (không lẫn Lưu/sao giả); tháng có Tứ Hóa thiếu mục tiêu chuyển `partial`; tháng nhuận bắt buộc Can Chi tường minh.
