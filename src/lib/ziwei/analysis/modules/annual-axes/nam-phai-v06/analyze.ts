...
import { scoreV06Domain } from "./score-domain";
...
/** Nam Phái Annual Axes V0.6 calibrated scoring core. */
export function analyzeAnnualAxesNamPhaiV06(chart: ChartData): AnnualAxesResult {
  ...
  const knowledge06Result = loadAnnualAxesKnowledgeV06NamPhai();
  if (!knowledge06Result.ok) {
    for (const issue of knowledge06Result.issues) {
      diagnostics.invalidKnowledge.push(`v0.6:${issue.path}: ${issue.message}`);
    }
    return invalidKnowledgeResult(chart.annualYear, diagnostics, "unavailable");
  }
  const knowledge06 = knowledge06Result.knowledge;
  ...
  if (!headFrame) {
    ...
    return {
      ...
      versions: {
        contractVersion: CONTRACT_VERSION,
        engineVersion: ENGINE_VERSION,
        knowledgeVersion: `annual-axes-v0.6@${knowledge06.calibration.formulaVersion}`,
      },
      status: "unavailable",
      axes,
      diagnostics: dedupeAnnualAxesDiagnostics(diagnostics),
  ...
  for (const domainDefinition of knowledge04.axisDefinitions.domains) {
    ...
    axes[domain] = {
      ...
      annualDelta: Math.round((scored.score - knowledge06.scoreProfile.neutral) * 10) / 10,
      natalResponse,
      activationGate: scored.activationGate,
      latent: scored.latent,
      scoreTrace: {
        formulaVersion: "v0.6-annual-dominant-core",
        candidateId: scored.trace.candidateId,
        signedLayerFactors: scored.trace.signedLayerFactors,
        directSupportRawBeforeLayerFactor: scored.trace.directSupportRawBeforeLayerFactor,
        directPressureRawBeforeLayerFactor: scored.trace.directPressureRawBeforeLayerFactor,
        tp4cSupportRawBeforeLayerFactor: scored.trace.tp4cSupportRawBeforeLayerFactor,
        tp4cPressureRawBeforeLayerFactor: scored.trace.tp4cPressureRawBeforeLayerFactor,
        directSupportRawAfterLayerFactor: scored.trace.directSupportRawAfterLayerFactor,
        directPressureRawAfterLayerFactor: scored.trace.directPressureRawAfterLayerFactor,
        tp4cSupportRawAfterLayerFactor: scored.trace.tp4cSupportRawAfterLayerFactor,
        tp4cPressureRawAfterLayerFactor: scored.trace.tp4cPressureRawAfterLayerFactor,
        spatialSigned: scored.trace.spatialSigned,
        annualActivationRaw: scored.trace.annualActivationRaw,
        activationGate: scored.trace.activationGate,
        natalGain: scored.trace.natalGain,
        latent: scored.trace.latent,
        domainScale: scored.trace.domainScale,
        absoluteScore: scored.trace.absoluteScore,
      },
