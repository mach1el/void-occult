...
import type { AnnualAxesKnowledgeV06NamPhai, AnnualAxisCalibrationV06 } from "./schema";
import {
...
import { scoreV06ChartDomains } from "../../../modules/annual-axes/nam-phai-v06/score-chart";

export const V06_CALIBRATION_GENERATED_AT = "2026-07-21T00:00:00.000Z";
export const V06_FORMULA_VERSION = "v0.6-annual-dominant-core";
export const V06_ACTIVATION_TARGET_GATE = 0.7;
...
export interface V06CalibrationSample {
  chartId: string;
  chartIndex: number;
  annualYear: number;
...
): V06CalibrationSample[] {
  const bases = buildAuditBirthInputs(contract);
  const samples: V06CalibrationSample[] = [];
...
function deriveActivationScale(positiveRaw: number[]): number {
  ...
  const target = Math.atanh(V06_ACTIVATION_TARGET_GATE);
...
export function deriveV06Calibration(
  ...
  return {
    schemaVersion: "0.6.0",
    profileId: "annual-axis-calibration-nam-phai-v0-6",
    formulaVersion: V06_FORMULA_VERSION,
    ...
    activationTargetMedianGate: V06_ACTIVATION_TARGET_GATE,
    activationScale,
