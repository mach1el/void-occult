import type {
  ...
  AnnualAxisCalibrationV06,
  AnnualBucketFormulaV06,
  AnnualDistributionGatesV06,
  AnnualEvidenceDedupePolicyV06,
  AnnualNatalGainV06,
  AnnualScoreProfileV06,
  AnnualSpatialBudgetV06,
} from "./schema";
import type { AnnualAxisDomainId } from "../schema";

export interface AnnualKnowledgeV06ValidationIssue {
  path: string;
  message: string;
}
...
function issue(path: string, message: string): AnnualKnowledgeV06ValidationIssue {
  return { path, message };
}
...
function validateSpatialBudget(
  budget: AnnualSpatialBudgetV06,
  resolvedSourceIds: Set<string>,
  issues: AnnualKnowledgeV06ValidationIssue[],
): void {
...
function validateDedupe(
  policy: AnnualEvidenceDedupePolicyV06,
  resolvedSourceIds: Set<string>,
  issues: AnnualKnowledgeV06ValidationIssue[],
): void {
  if (
...
function validateBucketFormula(
  formula: AnnualBucketFormulaV06,
  resolvedSourceIds: Set<string>,
  issues: AnnualKnowledgeV06ValidationIssue[],
): void {
  ...
  if (
    ...
    issues.push(
      issue(
        ...
        "V0.6 requires supportWeight=0, pressureWeight=0, activationWeight=1",
      ),
    );
  }
...
export function validateAnnualAxesKnowledgeV06NamPhai(
  knowledge: AnnualAxesKnowledgeV06NamPhai,
  resolvedSourceIds: Set<string>,
): { ok: true } | { ok: false; issues: AnnualKnowledgeV06ValidationIssue[] } {
  const issues: AnnualKnowledgeV06ValidationIssue[] = [];
